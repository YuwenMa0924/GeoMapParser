...
import torch
# Force-disable CUDA and use CPU only (critical!)
...
# ===================== CPU-only model loading (error-free version) =====================
device = "cpu"  # Force CPU; no device detection is performed

# Lightweight CPU-only model + CPU optimization settings
processor = TrOCRProcessor.from_pretrained(
    "microsoft/trocr-small-stage1",
    use_fast=False

)
# Key parameter: low_cpu_mem_usage=True resolves the CPU meta-tensor error!
ocr_model = VisionEncoderDecoderModel.from_pretrained(
    "microsoft/trocr-small-stage1",
   # Required for CPU! Fixes the core error
).to(device)
...
# The original single-image path is commented out and replaced by a batch image input folder
# IMAGE_PATH = "path/to/test/813.jpg"
IMAGE_INPUT_DIR = "path/to/representative_100"  # folder of images for batch processing (please change to your own path)
...
# JSON export folder (previous configuration retained; may be modified)
EXPORT_JSON_DIR = "path/to/comparison_trocr/results"
# Added: metadata folder configuration (change point 1)
METADATA_DIR = "path/to/comparison_trocr/metadata"  # path for saving the metadata JSON

# Initialize the global utilities (avoids repeated initialization during batch processing and improves efficiency)
yolo_model = YOLO(MODEL_PATH)  # global YOLO model
...
# -------------------------- Utility function: retrieve the list of images for batch processing --------------------------
def get_image_list(input_dir):
    """Iterate over the input folder and return the paths and base names of all images in supported formats."""
    image_list = []
...
# -------------------------- Core function: process a single image (all previous logic encapsulated here) --------------------------
def process_single_image(image_path, image_name):
    ...
    :param image_name: file name of the image without suffix (used to generate the JSON file name)
    ...
    try:
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                # Retrieve the threshold of the corresponding class
        ...
        # Print all classes detected by the YOLO model
        ...
        print(f"\n[{image_name}]Total of {len(updated_boxes)} elements detected:")
        ...
        # Sub-function: OCR text extraction (fixed version: unused imports removed, image preprocessing added)
        # Sub-function: OCR text extraction (final fixed version: binds YOLO labels to resolve field-matching failures)
        # Sub-function: OCR text extraction (CPU-only fixes + label binding)
        def task_extract_text_inner(image_path_inner, text_boxes_inner):
            ...
            try:
                ...
                for idx, box_dict in enumerate(text_boxes_inner):
                    ...
                    label = box_dict.get("label", f"textbox_{idx + 1}")
                    if len(box) != 4:
                        continue
                    ...
                    cropped_processed = Image.fromarray(binary).convert("RGB")

                    # CPU inference, no device issues
                    pixel_values = processor(
                        images=cropped_processed,
                        return_tensors="pt"
                    ).pixel_values.to("cpu")
                    ...
                    ocr_results[f"{label}_{idx + 1}"] = text
                    print(f"✅ [{label}] recognition result {idx + 1}: {text}")
        ...
        def task_analyze_graphics_inner(image_path_inner, scale_region_boxes_inner):
            ...
            # 1. Read the overall image dimensions first (reuse the img loaded by CV2 to avoid opening it twice)
            img_height, img_width = img.shape[:2]
            # 2. Compute the center coordinates of the scale-bar box (a more accurate basis for position determination)
            ...
            # Horizontal structuring element, specifically targeting the scale bar
...
You are a professional map-legend analyzer. Carefully analyze this legend image and return a JSON array.
        ...
        2. 'key_type': type of the symbol (e.g. "color", "icon", "line", "pattern").
        3. 'key_data': specific data for the symbol. If it is 'color', provide the HEX color value; if it is 'icon' or 'line', give a short English description such as "black_triangle" or "dashed_line".
"""
                    ...
                    # === Debug: print the raw content (repr) to aid troubleshooting ===
                    print(f"[{image_name}][DEBUG] raw content repr of box {idx}:", repr(content)[:1000])

                    # ===== 4. Robust extraction of the inner JSON =====
                    legend_json = []

                    # 4a) If it looks like a Python dict string (outer single-quoted keys), try ast.literal_eval first
                    parsed_inner = None
                    try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(f"[{image_name}][WARN] JSON parsing failed for legend box {idx}:", e1, "| fallback error:", e2)
        ...
        # Execute the three extraction steps
...
1. title: Extract from the `title` field of `ocr_results`. If there are multiple titles, merge them after filtering. Pay attention to the semantic accuracy of the recognized title: avoid garbled text or semantically incoherent content and check it carefully.
...
"""
        ...
        # ========= Added: generate and save the metadata JSON (change point 2) =========
        try:
...
if __name__ == "__main__":
    ...
    # Added: pre-create the metadata folder (change point 3)
    if not os.path.exists(METADATA_DIR):
        os.makedirs(METADATA_DIR, exist_ok=True)
        print(f"Metadata folder pre-created: {METADATA_DIR}")

    # 3. Loop over and batch-process each image
    success_count = 0
    fail_count = 0
    ...
    print(f"\n" + "=" * 60)
    print(f"Batch processing completed! Total: {len(image_tasks)} images | Success: {success_count} images | Failed: {fail_count} images")
    print(f"All successfully generated JSON files are saved in: {EXPORT_JSON_DIR}")
    print(f"All successfully generated metadata JSON files are saved in: {METADATA_DIR}")  # change point 4
    print("=" * 60)